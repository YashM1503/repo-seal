...
from reposeal.docker_backend import (
...
EXPECTED_COMMAND_TEMPLATE_SHA256 = (
    "sha256:0f1fad1b16ec4b2c2917eb206fe0bb5374f97ca757b9cff37c48538227c7f021"
)
...
class DockerPolicyTests(unittest.TestCase):
    ...
    def test_command_has_exact_mounts_and_required_security_flags(self) -> None:
        with tempfile.TemporaryDirectory(prefix="reposeal-docker-plan-") as directory:
            root = Path(directory)
            probe = root / "probe.py"
            ...
            command = build_docker_command(
                ...
                container_name="reposeal-probe-" + "a" * 32,
            )
        ...
        self.assertIn("dst=/reposeal-isolation-probe.py,readonly", mounts[0])
        rendered = "\n".join(command)
        for forbidden in (
            "--privileged",
            "--network=host",
    ...
    def test_command_rejects_relative_paths_commas_and_arbitrary_names(self) -> None:
        policy = DockerIsolationPolicy()
        root = Path("/tmp/reposeal-docker-policy")
    ...
    def test_engine_floor_and_docker_environment_fail_closed(self) -> None:
        ...
        with patch.dict(
            os.environ,
            {
                ...
                "DOCKER_HOST": "tcp://remote.example:2376",
                "REPOSEAL_CONTROL": "kept",
            },
            clear=True,
        ):
            environment = _docker_cli_environment()
        self.assertEqual(environment, {"REPOSEAL_CONTROL": "kept"})
    ...
    def test_image_declared_volumes_are_rejected(self) -> None:
        ...
        with (
            patch(
                "reposeal.docker_backend._run_metadata_command",
                return_value=result,
            ),
    ...
    def test_below_floor_engine_is_rejected_before_work_root_creation(self) -> None:
        with tempfile.TemporaryDirectory(prefix="reposeal-engine-floor-") as directory:
            work_root = Path(directory) / "must-not-exist"
            with (
                patch(
                    "reposeal.docker_backend._resolve_docker_cli",
                    return_value=Path(__file__),
                ),
                patch(
                    "reposeal.docker_backend._inspect_local_docker_endpoint",
                    return_value="unix:///var/run/docker.sock",
                ),
                patch(
                    "reposeal.docker_backend._inspect_image",
                    return_value={},
                ),
                patch(
                    "reposeal.docker_backend._inspect_engine",
...
@unittest.skipUnless(
    os.environ.get("REPOSEAL_RUN_DOCKER_INTEGRATION") == "1",
    "set REPOSEAL_RUN_DOCKER_INTEGRATION=1 with the pinned image available",
)
class DockerLiveIntegrationTests(unittest.TestCase):
    def test_pinned_backend_passes_every_control_except_review(self) -> None:
        with tempfile.TemporaryDirectory(prefix="reposeal-docker-live-") as directory:
            root = Path(directory)
            try:
